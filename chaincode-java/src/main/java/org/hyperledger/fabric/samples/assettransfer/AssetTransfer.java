...
public final class AssetTransfer implements ContractInterface {
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String owner) {
        // This transaction will take an asset id and an owner, duplicate the asset and assign
        // the duplicate to the new owner while keeping the original asset with the previous original owner
        //check assetID is valid
        if (AssetExists(ctx, assetID)) {
            Asset original = ReadAsset(ctx, assetID); //getting the asset mapped to the given asset id
            Asset duplicate = CreateAsset(
                ctx,
                original.getAssetID() + "duplicate",
                original.getColor(),
                original.getSize(),
